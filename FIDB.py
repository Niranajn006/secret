...
def get_bond_info(driver, date_params):
    ...
    # STN data
    stn_col_name = ['STNDate','BondType' 'Symbol', 'SymbolOrder', 'RegistrationCode', ]


    # MTM data
    mtm_col_name = ['MTMDate', 'Symbol', 'SymbolOrder', 'CurrencyCode', 'RegistrationCode',
    ...
    for i in raw_mtm:
        if i['Symbol'] in exclude_symbol:
            continue
        ele = {
            'MTMDate': date_params, # date modify
            'Symbol': i['Symbol'].strip(),
    ...
    df_mtm['MTMDate'] = pd.to_datetime(df_mtm['MTMDate'], format='%Y/%m/%d').dt.date
    ...
    df_mtm = df_mtm.set_index('MTMDate')
    ...
    for i in raw_eod:
        if i['Symbol'] in exclude_symbol:
            continue
        ele = {
            'EODDate': date_params, # date modify
            'Symbol': i['Symbol'].strip(),
    ...
    df_eod['EODDate'] = pd.to_datetime(df_eod['EODDate'], format='%Y/%m/%d').dt.date
    df_eod['lastDate'] = pd.to_datetime(df_eod['lastDate'], format='%Y/%m/%d').dt.date
    df_eod = df_eod.set_index('EODDate')
    ...
    return df_mtm, df_eod, bond_master

#%% =============  Main Function ============= 
driver = webdriver.Chrome(r'D:\Project files\Data\chromedriver.exe')
mtm, eod, bond_master = get_bond_info(driver, '2019-02-04')

#%% ============= Test Insert Data ============= 

# merged = pd.merge(x_mtm, x_eod, how='outer', on='Symbol') -> d
